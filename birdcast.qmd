...
#| warning: false
slc_note <- tibble(
  date = mdy(c("04/27/2026", "04/17/2026", "04/09/2026", "04/01/2026", "03/15/2026", "03/01/2026")),
  note = c("Likely birds: Yellow-rumped Warbler, Common Tern, Eastern Towhee, American Tree Sparrow, Bufflehead", "Likely birds: Ruby-crowned Kinglet, Brown Thrasher, Eastern Towhee, Pine Warbler, Savannah Sparrow, Blue-winged Teal", "Likely birds: Pine Warbler, Ruby-crowned Kinglet, American Bittern, Eastern Towhee, Hermit Thrush, Yellow-rumped Warbler",
...
ggplotly(slc, tooltips = c("label", "text"))
```

This plot highlights that migration over St. Lawrence County increased throughout the data collection period, with 2 large spikes of over 4 million birds in late April. Alternatively, peak migration in March only reached around 600,000 birds. The species of birds flying over changed throughout the season. In early March, waterfowl like Buffleheads and Snow Geese were the primary migrants. By the end of April, the diversity of species traveling increased greatly to include passerines (songbirds) like thrushes, sparrows, and warblers. The high spikes of over 4 million birds highlight how crucial the North Country environment is for migrating birds. This area is a summer home for many breeding songbirds, and is an important stopover site for birds migrating to their Arctic feeding grounds. 
...
fc <- ggplot(data = bird_cast_fc, aes(x = date, y = birds, label = flight_speed)) +
  geom_line(color = "darkgreen") +
  geom_point(size = 0.5, alpha = 0.7) +
  geom_point(data = filter(bird_cast_fc_annotated, !is.na(note)),
    aes(text = note),
    size = 3, shape = 23, color = "darkgreen", fill = "lightgreen", stroke = 1.2) +
    scale_y_continuous(labels = scales::comma) +
  scale_x_date(date_breaks = "6 days", date_labels = "%b %d") +
...
ggplotly(fc, tooltips = c("label", "text"))
```

In contrast to the St. Lawrence County plotly, Fairfax County (in Virginia) showed much more variability in the number of migrating birds passing over. Virginia's spring has constantly changing weather patterns that contribute to the high level of variability. Fairfax County also saw returning warblers and sparrows earlier than in St. Lawrence County. 

### Monroe County Birds by Day
```{r}
#| warning: false
mc_note <- tibble(
  date = mdy(c("05/01/2026", "04/18/2026", "04/01/2026", "03/15/2026", "03/01/2026")),
  note = c("Likely birds: Hooded Warbler, Bay-breasted Warbler, Gray-cheeked Thrush, White-rumped Sandpiper", "Likely birds: Scarlet Tanager, Swainson's Thrush, Yellow-billed Cuckoo, Bobolink, Common Nighthawk, Baltimore Oriole", "Likely birds: Yellow-rumped Warbler, Prothonotary Warbler, Red-breasted Merganser, Savannah Sparrow",
...
mc <- ggplot(data = bird_cast_mc, aes(x = date, y = birds, label = flight_speed)) + 
  geom_line(color = "darkgreen") +
  geom_point(size = 0.5, alpha = 0.7) +
  geom_point(data = filter(bird_cast_mc_annotated, !is.na(note)),
    aes(text = note),
    size = 3, shape = 23, color = "darkgreen", fill = "lightgreen", stroke = 1.2) +
  scale_y_continuous(labels = scales::comma) +
  scale_x_date(date_breaks = "6 days", date_labels = "%b %d") +
...
ggplotly(mc, tooltips = c("label", "text"))
```

In Monroe County, located on the southern tip of Florida, the number of nighttime migrants began increasing in mid-April, spiking on April 18th with around 630,000 birds. Florida has much milder and warmer weather than New York and Virginia, which contributes to the relatively higher stability. Additionally, the species of birds passing through Monroe County had some differences from the other two counties I looked at. There were several species that do not make it as far north as St. Lawrence County, like the Prothonotary Warbler and the Gray-cheeked Thrush. 
...
### Relationship Between Temperature and Number of Birds
```{r}
#| warning: false
...
### Relationship Between Precipitation and Number of Birds
```{r}
#| warning: false
mod_l <- lm(birds ~ prcp:location, data = bird_weather) 
mod_l |> tidy()

pred_l <- ggpredict(mod_l, terms = c("prcp", "location")) |>
  as.data.frame(terms_to_colnames = TRUE) |>
  as_tibble()

ggplot(data = bird_weather, aes(x = prcp, y = birds)) +
  geom_point() +
  geom_line(data = pred_l, aes(x = prcp, y = predicted), color = "darkgreen", linewidth = 1.2) +
    geom_ribbon(data = pred_l, aes(y = predicted,
                                  ymin = conf.low,
                                  ymax = conf.high), 
              alpha = 0.2) +
  ylim(0, 2000000) +
  theme_minimal() +
  facet_wrap(~location) +
  labs(title = "Relationship between daily precipitation and number of migratory birds", x = "Daily Precipitation", y = "Number of Birds")
```
...
My preliminary investigation of the relationship between number of birds and daily precipitation was to look at a linear regression. However, I didn't think that looked accurate, so I decided to run a negative binomial regression instead. A negative binomial regression accounts for the over dispersion of migration data where there are some large flight nights and many quiet ones, as well as some nights with heavy precipitation but most nights with very little. After performing the AIC test on both models, I was able to determine that the negative binomial model was better. This plot shows a clear decrease in migration when precipitation is occurring in all three counties. Active migration nights only occurred when precipitation was 0. The number of birds flying was very low when there was more than 0.5 inches of precipitation. Monroe County had the least steep slope because it had lower numbers of bird migration overall; however, it still highlights the trend of more precipitation resulting in less birds per night.


## Conclusion

### Final Analysis
The number of migratory birds looked very different in each county I looked at. I was especially interested by the difference between St. Lawrence County and Monroe County, with St. Lawrence having peak nights that were around 4 million birds higher. The high variability in Fairfax County was also interesting to me, which indicated that the region likely experiences more ups and downs in weather. Precipitation and temperature both had an impact on migration. Higher precipitation indicated lower migration levels, while higher temperature was associated with more migratory birds. 
...
Question asked in Claude AI: How can I make my plot look more readable?
The answer provided me with the `scale_y_continuous(labels = scales::comma)` and `scale_x_date(date_breaks = "6 days", date_labels = "%b %d")` which I used to change the y-axis to have commas instead of scientific notation, and the x-axis to more clearly show the dates. 
